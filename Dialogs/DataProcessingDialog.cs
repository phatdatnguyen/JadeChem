namespace JadeChem.Dialogs
{
    public partial class DataProcessingDialog : Form
    {
        ...
        public Dictionary<string, Dictionary<string, (double, double)>> FeatureScalersDictionary { get; set; }
        ...
        #region Constructor
        public DataProcessingDialog(Dictionary<string, Dictionary<string, (double, double)>> inputScalersDictionary, Dictionary<string, Dictionary<string, (double, double)>> featureScalersDictionary, Dictionary<string, Dictionary<string, (double, double)>> outputScalersDictionary, Dictionary<string, Dictionary<string, double>> dimensionalityReductionStepsDictionary)//,Dictionary<string,Dictionary<string,uint>> fingerprintExtractionDictionary)
        {
            InitializeComponent();

            InputScalersDictionary = inputScalersDictionary;
            FeatureScalersDictionary = featureScalersDictionary;
        ...
        private void DataProcessingDialog_Load(object sender, EventArgs e)
        {
            ...
            foreach (string columnName in FeatureScalersDictionary.Keys)
                columnListBox.Items.Add(columnName);
            ...
            foreach (ListViewItem item in processingStepsListView.Items)
            {
                if (item.Group.Header == "Scaling")
                {
                    ...
                    if ((InputScalersDictionary.ContainsKey(columnListBox.Text) && InputScalersDictionary[columnListBox.Text].ContainsKey(item.Text))
                        || (FeatureScalersDictionary.ContainsKey(columnListBox.Text) && FeatureScalersDictionary[columnListBox.Text].ContainsKey(item.Text))
...

                    {
                        ...
                        if (item.Text == "Min-max scaling")
                        {
                            ...
                            {
                                (double minOutput, double maxOutput) = FeatureScalersDictionary[columnListBox.Text][item.Text];
                                item.SubItems[1].Text = "{ range=(" + minOutput.ToString() + ", " + maxOutput.ToString() + ") }";
                            }
        ...
        private void EditButton_Click(object sender, EventArgs e)
        {
            ...
            if (processingStepsListView.SelectedItems[0].Text == "Min-max scaling")
            {
                ...
                if (InputScalersDictionary.ContainsKey(columnListBox.Text))
                    ...
                    (editParametersDialog.Parameters["Min output"], editParametersDialog.Parameters["Max output"]) = FeatureScalersDictionary[columnListBox.Text]["Min-max scaling"];
                else if (OutputScalersDictionary.ContainsKey(columnListBox.Text))
                ...
                if (editParametersDialog.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    if (InputScalersDictionary.ContainsKey(columnListBox.Text))
                        ...
                        FeatureScalersDictionary[columnListBox.Text]["Min-max scaling"] = (minOutput, maxOutput);
                    else if (OutputScalersDictionary.ContainsKey(columnListBox.Text))
